...
class CiscoConf(BasicConf):
  ...
  def _parse_vlans(cls, cp):
    ...
    for o in cp.find_objects(r"^\s*vlan\s+([0-9,-]+)"):
      ...
      for vids in cls._normalize_list(vids):
        vid = int(vids)
...
class ProCurveConf(BasicConf):
  ...
  def _parse_ifaces(cls, cp):
    ...
    for o in cp.find_objects(r"^\s*trunk"):
      ...
      if mi:
        ...
        for name in cls._normalize_list(mi.group(1)):
          if not name in ifaces:
            ifaces[name] = OrderedDict()
  ...
  def _parse_vlans(cls, cp, ifaces):
    ...
    for o in cp.find_objects(r"^\s*vlan\s+([0-9,-]+)"):
      ...
      for vidstr in cls._normalize_list(vidstr):
        vid = int(vidstr)
        ...
        for c in o.children:
          ...
          if m:
            for t in cls._normalize_list(m):
              ifname = str(t)
  ...
  def _parse_ifaces(cls, cp, active_vlans):
    ...
    for o in cp.find_objects(r"^\s*interface"):
      ...
      for c in o.children:
        ...
        m = c.re_match_typed(r"^\s*switchport trunk allowed vlan add\s+(.+)$")
        if m:
          ifaces[name]['allowed_vlan'] += self._normalize_list(m)
          continue
